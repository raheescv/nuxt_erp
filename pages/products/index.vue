...
const { data: list, refresh } = await useLazyAsyncData('list', () => {
    ...
    return $fetch(`${config.public.apiBaseUrl}products/get`, {
        method: "POST",
        body: {
            query: search.value,
            page: page.value,
    ...
    default: () => ({ data: [], total: 0, page: 1 }),
    watch: [page, search, searchStatus, pageCount, sort],
});

const todos = computed(() => list.value?.data || []);
pageTotal.value = list.value?.total || 0;
</script>
<template>
    <div
                    ...
                    <div class="col-span-12 mt-8">
                        ...
                        <UCard class="w-full" :ui="{
                            ...
                            <div class="flex items-center justify-between gap-3 px-4 py-3">
                                ...
                                <USelectMenu v-model="selectedStatus" :options="todoStatus" multiple
                            ...
                            <div class="flex justify-between items-center w-full px-4 py-3">
                                <div class="flex items-center gap-1.5">
                                    ...
                                    <USelect v-model="pageCount" :options="[3, 5, 10, 20, 30, 40]" class="me-2 w-20"
                                        size="xs" />
                                </div>
                            ...
                            <UTable v-model="selectedRows" v-model:sort="sort" :rows="todos" :columns="columnsTable"
                                :loading="status === 'pending'" sort-asc-icon="i-heroicons-arrow-up"
                                ...
                                <template #actions-data="{ row }">
                                    <UButton v-if="!row.completed" icon="i-heroicons-check" size="2xs" color="emerald"
                                        variant="outline" :ui="{ rounded: 'rounded-full' }" square />

                                    <UButton v-else icon="i-heroicons-arrow-path" size="2xs" color="orange"
                                        variant="outline" :ui="{ rounded: 'rounded-full' }" square />
                                </template>
                            </UTable>
